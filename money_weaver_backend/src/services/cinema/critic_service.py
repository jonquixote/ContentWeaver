...
def critique_plan_for_render(timing_plan, clip_durations: list[float],
    ...
    both paths). When clip_paths are given, synthesized shots get resolvable
    local:{i}:{path} ids so static mode actually scores frames; otherwise
    clip_{i} ids (unresolvable → zero frames → silent skip)."""
    ...
    for i, d in enumerate(clip_durations):
        if clip_paths and i < len(clip_paths) and clip_paths[i]:
            cid = f"local:{i}:{clip_paths[i]}"
        else:
            cid = f"clip_{i}"
        shots.append(TimelineShot(clip_id=cid, in_point_s=0.0, out_point_s=float(d)))
    return TimelinePlan(mode=MontageMode.OVERTONAL, shots=shots)
