...
def test_get_tasks_filters_by_project_id(client, auth_headers):
    ...
    body = client.get(f'/api/tasks?project_id={p1}', headers=auth_headers).json()
...
def test_create_task_persists_row_with_ownership(client, db_session, auth_headers):
    p1 = _create_project(client, auth_headers)
    ...
    task = db_session.get(Task, body['id'])
...
def test_update_task_happy_path(client, db_session, auth_headers):
    p1 = _create_project(client, auth_headers)
    created = _create_task(client, auth_headers, p1)
...
def test_update_task_non_object_body_is_400(client, auth_headers):
    ...
    r = client.put(f"/api/tasks/{created['id']}", headers=headers, data='null')
...
def test_delete_task_happy_path(client, db_session, auth_headers):
    p1 = _create_project(client, auth_headers)
    created = _create_task(client, auth_headers, p1)
...
def test_get_task_resolves_result_media_keys(client, auth_headers):
    ...
    _set_task(client, auth_headers, created['id'],
              result=json.dumps({'video_url': 'videos/a.mp4'}))
    body = client.get(f"/api/tasks/{created['id']}", headers=auth_headers).json()
...
def test_media_resolution_falls_back_on_storage_outage(client, auth_headers, monkeypatch):
    ...
    task = client.get(f"/api/tasks/{created['id']}", headers=auth_headers).json()
