...
def test_update_project_happy_path(client, db_session, auth_headers):
...
def test_update_project_non_object_body_is_400(client, auth_headers):
    ...
    r = client.put(f'/api/projects/{project_id}', headers=headers,
                   data='null')
    assert r.status_code == 400


def test_delete_project_happy_path(client, db_session, auth_headers):
