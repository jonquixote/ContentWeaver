...
def _add_voice(app, user_id, name='A voice'):
...
def test_cannot_read_other_users_project(client, auth_headers):
    ...
    a_project_id = created.get_json()['id']
    b_headers = _register_user(client, 'idor@test.com', 'idoruser')
    assert client.get('/api/projects', headers=b_headers).get_json() == []
...
def test_cannot_update_other_users_project(client, auth_headers):
    ...
    assert client.get(f'/api/projects/{a_project_id}',
                      headers=auth_headers).get_json()['title'] == 'A project'
...
def test_cannot_read_other_users_voice(client, app, auth_headers):
    a_id = _user_id(client, auth_headers)
    a_voice_id = _add_voice(app, a_id)
    b_headers = _register_user(client, 'idor@test.com', 'idoruser')
    voices = client.get('/api/voices', headers=b_headers).get_json()
    assert voices == []
    assert all(v['id'] != a_voice_id for v in voices)


def test_cannot_delete_other_users_voice(client, app, auth_headers):
    a_id = _user_id(client, auth_headers)
    a_voice_id = _add_voice(app, a_id)
    b_headers = _register_user(client, 'idor@test.com', 'idoruser')
    assert client.delete(f'/api/voices/{a_voice_id}',
                         headers=b_headers).status_code == 403
    with app.app_context():
        assert db.session.get(Voice, a_voice_id) is not None


def test_cannot_preview_other_users_voice(client, app, auth_headers):
    a_id = _user_id(client, auth_headers)
    a_voice_id = _add_voice(app, a_id)
    b_headers = _register_user(client, 'idor@test.com', 'idoruser')
